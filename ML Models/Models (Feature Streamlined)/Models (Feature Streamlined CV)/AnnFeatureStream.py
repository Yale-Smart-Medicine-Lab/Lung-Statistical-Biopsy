"""
This file contains the code to train and evaluate an Artificial Neural Network (ANN) model on the PLCO, UKB, and subsets of NHIS data.
"""
...
def calculate_metrics(y_true, y_pred):
    ...
    tn, fp, fn, tp = confusion_matrix(y_true, y_pred_labels).ravel() # fp and tp are 0 
...
def cross_validate_and_train(plco_data_path, ukb_data_path, nhis_data_path):
    ...
    model = create_model(X_plco.shape[1])
    model.fit(X_plco, plco_data['lung'], epochs=10, batch_size=1024, verbose=2)

    # Evaluate on PLCO training set
    model.evaluate(X_plco, plco_data['lung'], verbose=2, batch_size=1024)
    y_pred_plco_train = model.predict(X_plco)
...
plt.title('Neural Network: ROC Curves for Lung Cancer Prediction (chosen) CV')
plt.legend(loc='lower right')
plt.savefig('/Users/teresanguyen/Lung-Statistical-Biopsy/ML Models/Models (Feature Streamlined)/Models (Feature Streamlined CV)/feature streamlined cv photos/AnnFStreamlinedCvChosen.png', dpi=300, bbox_inches='tight')
plt.show()
