...
# Load NHIS data to determine common features (only features that have 70% of values are present)
...
# Keep only the common features and target in PLCO data
...
plt.title('PLCO Shallow Female: ROC curve (Log Reg)')
plt.legend(loc='lower right')
plt.savefig('PLCO_female_lung_ROC_logistic_shallow.png', dpi=300, bbox_inches='tight')
plt.show()

# load UKB data
UKB_data = pd.read_csv('Data Files/UKB_Female_Lung_Imputed_MAIN.csv')
X_ukb = UKB_data[X_plco.columns]
y_ukb = UKB_data['lung']
...
plt.title('UKB Shallow Female: ROC curve (Log Reg)')
plt.legend(loc='lower right')
plt.savefig('UKB_female_lung_ROC_logistic_shallow.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.xlabel('False positive rate')
plt.ylabel('True positive rate')
plt.title('NHIS Shallow Female: ROC curve (Log Reg)')
plt.legend(loc='lower right')
plt.savefig('NHIS_female_lung_ROC_logistic_shallow.png', dpi=300, bbox_inches='tight')
plt.show()