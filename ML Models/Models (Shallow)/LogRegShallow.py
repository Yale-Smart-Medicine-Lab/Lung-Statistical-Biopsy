...
def logistic_regression_analysis(plco_basepath, ukb_basepath, nhis_path, gender, title, plot_color='blue'):
    ...
    def plot_roc_curve(y_true, y_scores, type, title, filename):
        ...
        plt.title(f'Log Reg {type} {gender} {title}: ROC curve')
    ...
    plot_roc_curve(y_test, y_pred_plco, 'PLCO', title, 'PLCO_lung_ROC_logistic.png')

    # Evaluate on UKB and plot ROC
    y_pred_ukb = model.predict_proba(X_ukb)[:, 1]
    plot_roc_curve(y_ukb, y_pred_ukb, 'UKB', title, 'UKB_lung_ROC_logistic.png')

    # Evaluate on NHIS and plot ROC
    y_pred_nhis = model.predict_proba(X_nhis)[:, 1]
    plot_roc_curve(y_nhis, y_pred_nhis, 'NHIS', title, 'NHIS_lung_ROC_logistic.png')
