...
# Define a function to load data, train the model, and calculate ROC curve metrics
...
# Process UKB data using trained models and the same features used in PLCO training
...
plt.plot(fpr_plco_f, tpr_plco_f, label=f'PLCO Female (AUC = {auc_plco_f:.3f})', color='blue')
plt.plot(fpr_plco_m, tpr_plco_m, label=f'PLCO Male (AUC = {auc_plco_m:.3f})', color='green')
plt.plot(fpr_ukb_f, tpr_ukb_f, label=f'UKB Female (AUC = {auc_ukb_f:.3f})', color='red')
...
plt.savefig('ML Models/Models (Deep)/Feature Rich Photos/LogRegFeatureRich.png', dpi=300, bbox_inches='tight')
plt.show()
